...
abstract class Reports extends Page implements HasTable
{
    ...
    protected function getHeaderActions(): array
    {
        $actions = [
            Action::make('generateReport')
                ...
                ->action(function (array $data): void {
                    ...
                    if (! $user->can('create', ReportRun::class)) {
                        Notification::make()->title('You are not authorized to generate reports.')->danger()->send();

                        return;
                    }

                    $organizationId = $this->resolveActionOrganizationId($data);
        ...
        if (! $this->canManageReportSettings()) {
            return $actions;
        }

        $actions[] = Action::make('reportSettings')
            ->label('Settings')
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('organization.name')
                    ...
                    ->searchable()
                    ->sortable()
                    ->visible($this->shouldShowOrganizationControls()),
            ...
            ->filters([
                ...
                SelectFilter::make('organization_id')
                    ...
                    ->visible(fn (): bool => $this->shouldShowOrganizationControls() && count($this->organizationOptions()) > 1),
            ])
            ->recordActions([
                Action::make('download')
                    ...
                    ->color('success')
                    ->visible(fn (ReportRun $record): bool => $record->isDownloadable() && (Auth::user()?->can('view', $record) ?? false))
                    ->url(fn (ReportRun $record): string => $this->downloadUrl($record))
                    ->openUrlInNewTab(),
                ...
                Action::make('delete')
                    ...
                    ->visible(fn (ReportRun $record): bool => $this->canDeleteReports() && (Auth::user()?->can('delete', $record) ?? false))
     ...
     */
    protected function generateReportSchema(): array
    {
        return [
     ...
     */
    protected function deviceOptions(mixed $organizationId): array
    {
        $resolvedOrganizationId = $this->resolveFormOrganizationId($organizationId);
     ...
     * @return array<int|string, string>
     */
    protected function parameterOptionsForSelection(mixed $deviceId, mixed $reportTypeValue): array
    {
        ...
        $device = Device::query()
            ->whereIn('organization_id', $this->accessibleOrganizationIds())
            ->find((int) $deviceId);
     ...
     */
    protected function reportTypeOptionsForDevice(mixed $deviceId): array
    {
        if (! is_numeric($deviceId)) {
            return [];
        }
     ...
     * @return array<string, string>
     */
    protected function aggregationWindowOptionsForSelection(mixed $organizationId, mixed $reportTypeValue): array
    {
        ...
        if ($resolvedOrganizationId !== null) {
            foreach ($this->resolveShiftSchedulesForOrganization($resolvedOrganizationId) as $shiftSchedule) {
    ...
    protected function aggregationWindowHelperText(mixed $organizationId, mixed $reportTypeValue): ?string
    {
        $reportType = $this->normalizeReportTypeValue($reportTypeValue);
        $resolvedOrganizationId = $this->resolveFormOrganizationId($organizationId);
        ...
        if ($this->resolveShiftSchedulesForOrganization($resolvedOrganizationId) !== []) {
            return null;
        }
    ...
    protected function defaultAggregationWindowForSelection(?int $organizationId, string $reportTypeValue): ?string
    {
    ...
    protected function requiresAggregationWindowForType(string $reportTypeValue): bool
    {
        return in_array(
            $reportTypeValue,
     ...
     * }
     */
    protected function resolveAggregationSelection(mixed $value, int $organizationId): array
    {
     ...
     * }>
     */
    protected function resolveShiftSchedulesForOrganization(int $organizationId): array
    {
    ...
    protected function shiftScheduleOptionValue(string $scheduleId): string
    {
     ...
     * }
     */
    protected function defaultGenerateReportFormData(?int $organizationId = null): array
    {
     ...
     * }
     */
    protected function defaultSettingsFormData(?int $organizationId = null): array
    {
        $resolvedOrganizationId = $organizationId ?? $this->defaultOrganizationId();
        $settings = $resolvedOrganizationId !== null ? $this->settingsForOrganization($resolvedOrganizationId) : null;
    ...
    protected function settingsForOrganization(int $organizationId): ?OrganizationReportSetting
    {
        if (! in_array($organizationId, $this->accessibleOrganizationIds(), true)) {
            return null;
        }
    ...
    protected function normalizeReportTypeValue(mixed $value): string
    {
    ...
    protected function extractReportTypeValue(mixed $value): ?string
    {
        if ($value instanceof ReportType) {
            return $value->value;
        }
    ...
    protected function normalizeReportGroupingValue(mixed $value): string
    {
        if ($value instanceof ReportGrouping) {
            return $value->value;
        }
    ...
    protected function formatBytes(mixed $value): string
    {
    ...
    protected function resolveDefaultTimezone(): string
    {
    ...
    protected function resolveDefaultMaxRangeDays(): int
    {
    ...
    public static function canAccess(): bool
    {
        ...
        return $user instanceof User && $user->can('viewAny', ReportRun::class);
    }
    ...
    {
        return is_numeric($data['organization_id'] ?? null) ? (int) $data['organization_id'] : 0;
    }
    ...
    {
        return route('reporting.report-runs.download', ['reportRun' => $reportRun]);
    }

    protected function resolveActionErrorMessage(Throwable $exception): string
    {
